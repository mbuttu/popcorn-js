 ...
 */
  ...
  Popcorn.plugin( "slideout", {
      manifest: {
        about: {
          name: "Popcorn slideout Plugin",
          version: "0.1",
          author: "Scott Downe",
      ...
      _setup: function( options ) {
        ...
        options.slideout = document.createElement( "div" );
        options.slideout.classList.add( "slideout" );

        options.info = document.createElement( "div" );
        options.info.classList.add( "info" );

        var title = document.createElement( "span" ),
            spacer = document.createElement( "div" ),
            description = document.createElement( "div" );

        spacer.classList.add( "info-spacer" );
        ...
        options.info.appendChild( title );
        options.info.appendChild( spacer );
        options.info.appendChild( description );

        options.slideout.appendChild( options.anchor );
        options.slideout.appendChild( options.info );
        ...
        if ( target ) {
          if ( [ "VIDEO", "AUDIO" ].indexOf( target.nodeName ) > -1 ) {
            ...
            options.trackedContainer.element.appendChild( options.slideout );
          }
          else {
            target && target.appendChild( options.slideout );
          }
        }
      ...
      start: function( event, options ) {
        options.slideout.classList.add( "slideout-active" );
       ...
       */
        ...
        options.slideout.classList.remove( "slideout-active" );
        if ( options.trackedContainer ) {
      ...
      _teardown: function( options ) {
        ...
        else if ( options.slideout.parentNode ) {
          options.slideout.parentNode.removeChild( options.slideout );
        }
      }
  });
})( Popcorn );
